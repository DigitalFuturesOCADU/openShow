---
id: 2904
slug: lets-keep-in-touch
title: Let's Keep in Touch
show: '2025'
year: 2025
session: null
affiliation: []
medium:
  - 3d-printing
  - code
  - digital-fabrication
  - mobile
  - physical-computing
tags: []
credits:
  - personId: olivia-pasian
    name: Olivia Pasian
    role: null
creditsRaw: Olivia Pasian
media:
  - type: image
    id: 3047
    file: 2025/12/LetsKeepinTouch_4_Olivia-Pasian-1-scaled_FEATURE.jpg
    role: featured
    bytes: 122804
    alt: null
    caption: null
  - type: image
    id: 2906
    file: 2025/12/LetsKeepinTouch_1_Olivia-Pasian.jpg
    bytes: 8855579
    alt: null
    caption: null
  - type: image
    id: 2909
    file: 2025/12/LetsKeepinTouch_4_Olivia-Pasian-1.png
    servedByWordpress: 2025/12/LetsKeepinTouch_4_Olivia-Pasian-1-scaled.png
    bytes: 6140262
    alt: null
    caption: null
  - type: image
    id: 2908
    file: 2025/12/LetsKeepinTouch_3_Olivia-Pasian.jpg
    bytes: 8502371
    alt: null
    caption: null
  - type: image
    id: 2907
    file: 2025/12/LetsKeepinTouch_2_Olivia-Pasian.png
    bytes: 4057004
    alt: null
    caption: null
layout: default
links: []
status: publish
sourceTerms:
  - 2025 3D Printing
  - 2025 Code
  - 2025 Digital Fabrication
  - 2025 Mobile
  - 2025 Physical Computing
wordpress:
  postDate: '2025-12-08 23:43:43'
  originalSlug: lets-keep-in-touch
  link: https://df.show/portfolio/items/lets-keep-in-touch
---

"This project is a prototype of a design research thesis project. The larger research project, and this prototype, both investigate the potential for haptic technology to help users “keep in touch” with their loved ones when apart. Eventually, the aim is to be able to send signals to your friends and family to feel connected. 

In this prototype, visitors are invited to sit with friends, family, or strangers to explore haptic communication. There are screen interfaces to design vibration patterns which can be output on the haptic devices. These devices can be held, worn, or placed anywhere on the body to experience the vibrational patterns. The pattern creation can be either a shared collaborative experience or a solo exploration and can focus on communication or felt experience. Users are prompted to think on shared memories, non-verbal communications, gestures, verbal phrases, or inside jokes, then asked – what would you want to send each-other via haptics? "
