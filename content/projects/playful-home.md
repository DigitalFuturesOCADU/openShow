---
id: 2947
slug: playful-home
title: Playful Home
show: '2025'
year: 2025
session: null
affiliation:
  - graduate
medium:
  - 3d-printing
  - code
  - digital-fabrication
  - physical-computing
  - sound
tags: []
credits:
  - personId: joshua-jacob-pothen
    name: Joshua Jacob Pothen
    role: null
creditsRaw: Joshua Jacob Pothen
media:
  - type: image
    id: 2948
    file: 2025/12/PlayfulHome_5_Joshua-Pothen.jpeg
    role: featured
    servedByWordpress: 2025/12/PlayfulHome_5_Joshua-Pothen-scaled.jpeg
    bytes: 7337368
    alt: null
    caption: null
  - type: image
    id: 2949
    file: 2025/12/PlayfulHome_2_Joshua-Pothen.jpeg
    bytes: 7190695
    alt: null
    caption: null
  - type: image
    id: 2950
    file: 2025/12/PlayfulHome_3_Joshua-Pothen.jpeg
    servedByWordpress: 2025/12/PlayfulHome_3_Joshua-Pothen-scaled.jpeg
    bytes: 6774254
    alt: null
    caption: null
layout: default
links:
  - label: Project
    url: https://joshuapothen.notion.site/joshua-thesis-log
    status: unchecked
status: publish
sourceTerms:
  - 2025 3D Printing
  - 2025 Code
  - 2025 Digital Fabrication
  - 2025 Physical Computing
  - 2025 Sound
  - Open Show 2025
wordpress:
  postDate: '2025-12-09 00:08:29'
  originalSlug: playful-home
  link: https://df.show/portfolio/items/playful-home
manualOverrides:
  - affiliation
---

"The project explores a new, creative perspective on the networked smart home infrastructures that support our daily lives. Commercially available devices such as voice activated assistants and automated lighting are mostly thought of as purely functional. 

This project seeks to understand how existing home infrastructure can be used as a foundation for entirely new experiences and objects that are playful and engaging, rather than just utilitarian.The goal isn't to change or modify a home's existing systems, but rather to design a separate, new system that works alongside them. The aim is to discover if these new experiences can be used to spark moments of play, curiosity, and fun.  "
