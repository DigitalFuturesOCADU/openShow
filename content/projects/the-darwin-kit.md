---
id: 1830
slug: the-darwin-kit
title: The Darwin Kit
show: '2020'
year: 2020
session: null
affiliation:
  - graduate
medium:
  - code
  - speculative-fiction
  - wearables
  - web
tags: []
credits:
  - personId: manisha-laroia
    name: Manisha Laroia
    role: null
creditsRaw: Manisha Laroia
media:
  - type: image
    id: 1831
    file: 2020/12/DarwinKit_1_Manisha-Laroia.jpg
    role: featured
    bytes: 173609
    alt: null
    caption: null
  - type: image
    id: 1832
    file: 2020/12/DarwinKit_2_Manisha-Laroia.png
    bytes: 1040413
    alt: null
    caption: null
  - type: image
    id: 1833
    file: 2020/12/DarwinKit_3_Manisha-Laroia.jpg
    bytes: 154651
    alt: null
    caption: null
layout: default
links:
  - label: Project
    url: https://www.manishalaroia.com/the-darwin-kit
    status: unchecked
status: publish
sourceTerms:
  - 2020 Code
  - 2020 Graduate
  - 2020 SpeculativeFiction
  - 2020 Wearables
  - 2020 Web
  - Open Show 2020
wordpress:
  postDate: '2020-12-15 07:39:04'
  originalSlug: the-darwin-kit
  link: https://df.show/portfolio/items/the-darwin-kit
---

The Darwin Kit, is a speculative artefact, an “at-home DNA self-editing kit”. Using the diegetic space of Speculative Ethics based on Dunne &amp; Raby's 'Speculative Everything', the approach through the making was to explore the future technology of self-editing one's DNA and creating a space where it was introduced in the world. In the possible future, the Darwin Kit can be bought from gene-mapping companies. The user experience is for the participant to wear the disc and start the process on the tablet which guides them through a fictitious DNA editing interface. The designed artefact has a disc-wearable with an Adafruit CPX that is used to start a Processing sketch as shown in the tablet.
